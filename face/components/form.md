...
>地址
>/components/form/bigForm.js

>引用
>import bigForm from '/components/form/bigForm.js'

>注册
>components 对象内注册 bigForm 和引用时的名字相同

>使用

```js
// html
<big-form :config="config" :render="render" :behavior="behavior" :event="event"></big-form>
// js
data(){
    return{
        config: {
